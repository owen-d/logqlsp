...
export async function activate(context: ExtensionContext) {
  let disposable = commands.registerCommand("helloworld.helloWorld", async uri => {
    ...
    const url = Uri.parse('/home/victor/Documents/test-dir/logql/another.logql')
  ...
  const traceOutputChannel = window.createOutputChannel("logql Language Server trace");
  const command = process.env.SERVER_PATH || "logql-language-server";
  ...
  let clientOptions: LanguageClientOptions = {
    ...
    documentSelector: [{ scheme: "file", language: "logql" }],
    synchronize: {
  ...
  client = new LanguageClient("logql-language-server", "logql language server", serverOptions, clientOptions);
